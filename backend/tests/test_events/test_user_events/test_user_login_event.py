...
class TestUserLoginEvent(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Register the login_user blueprint
        self.added_username = "test_user2"
        self.added_email = "test_user2@example.com"
        self.added_password = "valid%2_Password"
        with self.app.app_context():
            db.create_all()
            # Add a sample user with a hashed password
            test_user = User(
                username=self.added_username,
                email=self.added_email,
                password=self.added_password,
            )
    ...
    def test_successful_login(self):
        ...
        response = self.client.post(
            "/api/login_user/",
            json={"username": self.added_email, "password": self.added_password},
        )
    ...
    def test_invalid_credentials(self):
        ...
        response = self.client.post(
            "/api/login_user/",
            json={"username": "test_user@example.com", "password": "wrong_password"},
        )
